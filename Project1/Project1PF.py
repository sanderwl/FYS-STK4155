...
from FunctionsDef import inputsss, FrankeFunc, MSE, R2,StdPolyOLS,designMatrixFunc, designMatrixFunc2,\
    betaConfidenceInterval,Scale, sciKitSplit, MSEplot, calc_Variance, calc_bias, BVPlot, Scale2, realSplit,FrankPlot, bootStrap, CV, MSEplotCV
import scipy.stats
...
if (str(part) == "a" or str(part) == "all"):
    ...
    for i in range(poly):
        ...
        for j in range(int(B)):
            ...
            biasO, varianceO, MSE_train_polyO, MSE_test_polyO, randomNumber = bootStrap(designMatrix_p,z,n,scaleInp,testSize,poly,i,rn)
            bias[j] = biasO
    ...
    for i in CVN:
        ...
        bias, variance, MSE_train, MSE_test = CV(designMatrix,z,n,CVN[dexxer],scaleInp)
